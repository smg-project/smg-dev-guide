...
- There are 10 `LoadBalancingPolicy` impls (random, round_robin, passthrough, power_of_two, cache_aware, least_load, bucket, manual, consistent_hashing, prefix_hash). Match an existing one rather than inventing API. (`passthrough` is a single-backend routing policy; `dp_min_token.rs` implements the separate `DPRankLoadPolicy` trait and is not counted.)
